...
const smartBrandCase = (value = '') =>
    value.split(' ').map(smartCaseWord).join(' ');
...
const compactTitle = (rawTitle = '', { maxChars, maxWords, includeSpecs }) => {
    ...
    const usefulSpecs = includeSpecs
        ? segments.slice(1).filter((segment) => IMPORTANT_SPEC_REGEX.test(segment)).slice(0, 2)
        : [];

    let candidate = cleanSegment([core, ...usefulSpecs].join(' | '));
...
export const getCardTitle = (rawTitle = '') =>
    compactTitle(rawTitle, { maxChars: 58, maxWords: 9, includeSpecs: false });

export const getDisplayTitle = (rawTitle = '') =>
    compactTitle(rawTitle, { maxChars: 118, maxWords: 18, includeSpecs: true });
...
export const createProductTitleSet = (rawTitle = '') => {
    ...
    return {
        ...
        displayTitle: getDisplayTitle(originalTitle),
        cardTitle: getCardTitle(originalTitle)
    };
};
