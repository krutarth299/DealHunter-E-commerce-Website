...
const TIMER_REGEX = /\b\d{1,2}:\d{2}(?::\d{2})?\b/g;
const TITLE_JUNK_REGEX = /\b(?:deal price|mrp|save\s*₹?\d+|off\s*\d+%|\d+%\s*off|coupon|cashback|bank offer|emi|exchange|delivery by|free delivery|stock running out|wishlist|add to cart|login|share|reviews?)\b/i;
...
const stripNoise = (value = '') =>
  normalizeSpaces(value)
    ...
    .replace(PROMO_REGEX, ' ')
    .replace(/\b\d+%\b/gi, ' ')
    .replace(/\b(?:save|starts?|from)\s+at\b/gi, ' ')
    .replace(/[_*#]+/g, ' ')
    .replace(/[|]{2,}/g, ' ')
...
export const cleanFetchedProductTitle = (rawTitle = '') => {
  const titleSet = createProductTitleSet(rawTitle);
  return normalizeSpaces(titleSet.displayTitle || titleSet.cardTitle || titleSet.originalTitle || '');
};

export const isClearlyBrokenTitle = (rawTitle = '') => {
  const original = normalizeSpaces(rawTitle);
  const cleaned = cleanFetchedProductTitle(original);
  if (!cleaned || cleaned.length < 12) return true;
  if (TITLE_JUNK_REGEX.test(original) && cleaned.length < 16) return true;
  const normalizedOriginal = original.toLowerCase();
  const normalizedCleaned = cleaned.toLowerCase();
  if (normalizedOriginal && normalizedCleaned && normalizedOriginal !== normalizedCleaned) {
    const junkHits = [
      /(?:₹|rs\.?|inr)\s*\d/i,
      /\b\d+%\s*off\b/i,
      /\bdeal price\b/i,
    ...
    if (junkHits && cleaned.length < 14) return true;
  }
  return false;
};
