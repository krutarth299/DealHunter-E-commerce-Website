export async function extractCroma(page) {
    ...
    return await page.evaluate(() => {
        ...
        // 3. DOM Fallbacks
        if (!data.title) {
            data.title = getText('.pd-title') || getText('h1.pd-title') || getText('h1') || getText('.product-title');
        }

        if (!data.price) {
            data.price = getText('#pdp-mrp-price') || getText('.pdp-cp') || getText('.amount') || getText('.cp-price') || getText('.new-price') || getText('[data-testid="price"]') || getText('.price');
        }
        ...
        if (!foundMrp) {
            const possibleMrps = document.querySelectorAll('.pdp-mrp, span.mrp, del, .original-price, s, span[style*="line-through"], [class*="strike"]');
        ...
        if (!data.category) {
            const breadcrumbs = Array.from(document.querySelectorAll('.breadcrumb-item, .breadcrumbs li, .cr-breadcrumb li'))
        ...
        // Images fallback
        const domImgs = Array.from(document.querySelectorAll('img')).map(img => img.getAttribute('data-src') || img.src || img.getAttribute('src'));
        domImgs.forEach(src => {
            if (src && src.startsWith('http') && (src.includes('croma.com') || src.includes('product')) && !src.includes('lazyLoading')) {
                // Ignore small icons or svg
                if (!src.includes('placeholder') && !src.endsWith('.svg') && !src.includes('video_thumbnail')) {
                    // Strip the Croma resizing parameter for high-res images
